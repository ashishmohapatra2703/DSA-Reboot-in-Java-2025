...
class StockSpanner {
    // span = Number of consecutive previous days whose price is <= today's price.
    record PriceSpanPair(int price, int span) {}
    Stack<PriceSpanPair> recentPrevGreaterPrices;

    public StockSpanner() {
        recentPrevGreaterPrices = new Stack<>();
    }
    
    public int next(int price) {
        int span = 1; // itself
        while(!recentPrevGreaterPrices.empty() && recentPrevGreaterPrices.peek().price() <= price) {
            span += recentPrevGreaterPrices.peek().span();
            recentPrevGreaterPrices.pop();
        }

        recentPrevGreaterPrices.push(new PriceSpanPair(price, span));
        return span;
    }
}
