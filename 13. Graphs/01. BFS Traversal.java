...
class Solution {
    public ArrayList<Integer> bfs(ArrayList<ArrayList<Integer>> adj) {
        ...
        que.add(0);   //pushing first vertex in queue.
	    visited[0] = true; //mark first vertex as visited(true).
	    
	    while(!que.isEmpty())
	    {
	        int currentNode = que.poll();
	        bfsOfGraph.add(currentNode);
	        
	        for(int i: adj.get(currentNode)) //traversing over all the neighbours of currentNode
	        {
	            if(!visited[i])
	            {
	                que.add(i);
	                visited[i] = true; //if they aren't visited, add to queue and mark them visited
	            }
	        }
	    }
	    return bfsOfGraph;
